...
class covid_pay extends Component {
  constructor(props) {
    super(props);
  }
  componentDidMount() {
  ...
  getAITraining=()=>{
    //console.log('轮询测试 '+new Date())
      let url2 = "http://"+this.context.url+"/aicare-customer-api/customer/physicaltest/checkresult?orderId="+this.props.route.params.orderId;
              ...
              .then((json) => {
                if (json.code == 0) {
                  ...
                  if(json.data.payResult=="PAY_SUCCESS"){
                    this.props.navigation.navigate("covid_finish")
                  }
                } else {
  ...
  render() {
    ...
    /*if (newNavState.url.startsWith("https://mpay.royalpay.com.au/api/v1.0/rpaypmt_svc/deductions/")){
      ...
      if (newNavState.url.includes('?errors=true')) {
        this.webview.stopLoading();
        this.props.navigation.navigate("covid_info")
      }
...
covid_pay.contextType = DataContext;

export default covid_pay;