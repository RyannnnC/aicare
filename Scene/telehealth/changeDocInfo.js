...
export default function changeDocInfo({route,navigation}) {
  ...
  const { orgId, docId,doctype,address,docName,id,queryId } = route.params;
  ...
  useEffect(() => {
    ...
    const updateAppointment=(s_id)=>{
        let url2 = "http://"+user.url+"/aicare-customer-api/customer/user/update-appointment?scheduleDetailedId=".concat(s_id).concat("&id=").concat(id);
                fetch(url2,{
                  method: 'GET',
                  mode: 'cors',
                  credentials: 'include',
