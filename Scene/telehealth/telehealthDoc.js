...
export default class telehealthDoc extends Component {
    constructor(props) {
      super(props);
      //date: new Date();
      this.state={
      ...
      return (
        <View style={styles.card} key={item.id}>
          ...
          <TouchableOpacity onPress={() =>{
            this.props.navigation.navigate("docInfo")}}>
            <Image
          ...
          <View>
            ...
            <Text style={{fontSize:12, color:'#666666', fontWeight: '400'}}>{item.category + " - "+ item.clinic}</Text>
          </View>
          </View>
          <View style={{flexDirection: 'row',paddingBottom: 15, borderBottomWidth: 1, borderBottomColor:'#EEEEEE'}}>
            ...
            />
            <Text style={{fontSize:12, color:'#999999', fontWeight: '400'}}>{item.score+" ("+item.commentNum+"条评价)"}</Text>
          </View>
